...
class AIAgent:
    ...
    def update(self, dt):
        ...
        for camera in self.game_map.cameras:
            if camera.active and camera.can_see(self.pos[0], self.pos[1], self.game_map):
                self.captured = True
                self.is_moving = False
                return
        ...
        while queue:
            ...
            for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
                ...
                if (0 <= nx < self.game_map.size and 
                    ...
                    self.game_map.grid[nx][ny] == 0 and  # Chỉ đi trên đường đi, không leo tường
                    new_pos not in visited):
                    
                    # Check if the new position is visible by any camera
                    visible_to_camera = False
                    for camera in self.game_map.cameras:
                        if camera.active and camera.can_see(nx, ny, self.game_map):
    ...
    def render(self, screen):
        ...
        # Optionally render the path for debugging
        self.render_path(screen)
    ...
    def render_path(self, screen):
        """Render the calculated path for debugging."""
        if self.path and len(self.path) > 0:
            # Draw path as a series of dots
            for i, (x, y) in enumerate(self.path):
                ...
                screen_x = (y + 0.5) * self.cell_size
                screen_y = (x + 0.5) * self.cell_size
                
                # Draw dot with different colors based on progress
                if i < self.path_index:
                    color = (100, 100, 100)  # Gray for visited path
                else:
                    color = (0, 100, 200)  # Blue for upcoming path
                    
                # Draw small circle for path point
